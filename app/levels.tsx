      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="px-5 pb-28 pt-5">
        ...
        <FadeInView delay={60}>
          <View className="mt-4 rounded-[26px] bg-ink p-5 shadow-sm">
            <Text className="text-[10px] font-bold text-petal">LEVEL SYSTEM</Text>
            <Text className="mt-1 text-[20px] font-bold text-white">마이부기 성장 단계</Text>
              ...
              {levelSteps.map((step, index) => (
                <View key={step.level} className={`flex-row items-center py-3 ${index ? 'border-t border-white/10' : ''}`}>
                  <View className={`h-9 w-9 items-center justify-center rounded-full ${user.level >= step.level ? 'bg-berry' : 'bg-white/10'}`}>
                    <Text className="text-[11px] font-bold text-white">{step.level}</Text>
                  </View>
                  <View className="ml-3 flex-1">
                    <Text className="text-[13px] font-bold text-white">Lv.{step.level} {step.name}</Text>
        ...
        <FadeInView delay={120}>
          <View className="mt-4 rounded-[26px] bg-white p-5 shadow-sm">
            <Text className="text-[10px] font-bold text-berry">XP GUIDE</Text>
            <Text className="mt-1 text-[20px] font-bold text-ink">활동별 경험치</Text>
            <View className="mt-4">
              {xpActivities.map((activity, index) => (
                <View key={activity.id} className={`flex-row items-center py-3.5 ${index ? 'border-t border-line' : ''}`}>
                  ...
                  <Text className="ml-3 flex-1 text-[13px] font-bold text-ink">{activity.label}</Text>
                  <Text className="rounded-full bg-soft px-3 py-2 text-[11px] font-bold text-berry">+{activity.xp} XP</Text>
                </View>
              ))}
            </View>
          </View>
