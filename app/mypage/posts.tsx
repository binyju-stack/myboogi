...
function CategoryTab({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  return (
    <AnimatedPressable onPress={onPress} className={`mr-2 rounded-full border px-4 py-2.5 ${active ? 'border-[#FF2E6F] bg-[#FFF2F6]' : 'border-[#EEF2F6] bg-white'}`}>
      <Text className={`text-[13px] font-semibold leading-[18px] ${active ? 'text-[#FF2E6F]' : 'text-[#94A3B8]'}`}>{label}</Text>
    </AnimatedPressable>
  );
}
...
function Meta({ icon, label }: { icon: keyof typeof Ionicons.glyphMap; label: string }) {
  return (
    <View className="mr-4 flex-row items-center">
      ...
      <Text className="ml-1 text-[12px] font-normal leading-[18px] text-[#94A3B8]">{label}</Text>
    </View>
  );
}
...
function MyPostCard({ post, index }: { post: Post; index: number }) {
  ...
  return (
    <FadeInView delay={index * 45}>
      <AnimatedPressable onPress={() => router.push(`/community/${post.id}` as never)} className="border-b border-[#EEF2F6] bg-white px-5 py-4">
          ...
          <View className="flex-1 pr-3" style={{ minWidth: 0 }}>
            <View className="flex-row items-center">
              ...
              <View className="ml-2 flex-1">
                <Text className="text-[13px] font-bold leading-[18px] text-[#111827]" numberOfLines={1}>{post.author}</Text>
                <Text className="mt-0.5 text-[12px] font-normal leading-[18px] text-[#94A3B8]" numberOfLines={1}>{post.category} · {post.createdAt}</Text>
              </View>
            </View>
            <Text className="mt-3 text-[18px] font-bold leading-[26px] text-[#111827]" numberOfLines={2}>{post.title}</Text>
            <Text className="mt-1.5 text-[14px] font-medium leading-[22px] text-[#94A3B8]" numberOfLines={2}>{post.content}</Text>
          </View>
...
export default function MyPostsScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F7F8FA]" edges={['top']}>
      <View className="border-b border-[#EEF2F6] bg-white px-5 pb-4 pt-4">
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center">
            ...
            <Text className="text-[22px] font-bold leading-8 text-[#111827]">내가 작성한 글</Text>
          </View>
