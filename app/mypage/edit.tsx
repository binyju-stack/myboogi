...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <View className="mb-3 w-[48%] rounded-[18px] bg-soft px-4 py-3.5">
      ...
      <Text className="mt-1 text-[18px] font-bold text-ink">{value.toLocaleString()}</Text>
    </View>
  );
}
  ...
  return (
    <Page>
      ...
      <View className="bg-white px-5 pb-6 pt-4">
        <Text className="text-[10px] font-bold text-berry">MY PROFILE</Text>
        <Text className="mt-1 text-[24px] font-bold text-ink">내 정보를 깔끔하게 정리해요</Text>
      ...
      <View className="px-5 pt-5">
        <FadeInView>
          <View className="items-center rounded-[26px] border border-line bg-white p-5 shadow-sm">
            ...
            <AnimatedPressable onPress={() => Alert.alert('프로필 사진 변경 기능은 준비중입니다.')} className="mt-4 flex-row items-center rounded-full bg-blush px-4 py-3">
              ...
              <Text className="ml-1.5 text-[11px] font-bold text-berry">프로필 사진 변경</Text>
            </AnimatedPressable>
          </View>
        </FadeInView>
        ...
        <FadeInView delay={45}>
          <View className="mt-4 rounded-[26px] border border-line bg-white p-5 shadow-sm">
            <Text className="mb-4 text-[15px] font-bold text-ink">기본 정보</Text>
        ...
        <FadeInView delay={90}>
          <View className="mt-4 rounded-[26px] border border-line bg-white p-5 shadow-sm">
            <Text className="mb-4 text-[15px] font-bold text-ink">SNS 정보</Text>
        ...
        <FadeInView delay={135}>
          <View className="mt-4 rounded-[26px] border border-line bg-white p-5 shadow-sm">
            <Text className="text-[15px] font-bold text-ink">관심 품종</Text>
              ...
              {interestOptions.map((interest) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text className={`${selected ? 'ml-1 text-white' : 'text-muted'} text-[11px] font-bold`}>{interest}</Text>
                  </AnimatedPressable>
                );
              })}
            </View>
        ...
        <FadeInView delay={180}>
          <View className="mt-4 rounded-[26px] border border-line bg-white p-5 shadow-sm">
            <Text className="text-[15px] font-bold text-ink">활동 통계</Text>
        ...
        <AnimatedPressable onPress={() => Alert.alert('프로필 저장 기능은 준비중입니다.')} className="mt-5 items-center rounded-[20px] bg-berry py-4 shadow-sm">
          <Text className="text-[13px] font-bold text-white">프로필 저장</Text>
        </AnimatedPressable>
      </View>
    </Page>
  );
