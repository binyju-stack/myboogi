        ...
        <View className="rounded-t-[30px] bg-white px-5 pt-5 shadow-lg" style={{ paddingBottom: Math.max(insets.bottom, 16) }}>
          <View className="mb-5 flex-row items-center justify-between">
            <View className="flex-1 pr-4">
              <Text className="text-[10px] font-black text-berry">STATUS</Text>
              <Text className="mt-1 text-[22px] font-black text-ink">상태 변경</Text>
          ...
          {listingStatusOptions.map((option) => {
            ...
            return (
              <AnimatedPressable key={option.key} onPress={() => onChange(option.key)} className={`mb-3 flex-row items-center rounded-[20px] px-4 py-4 ${selected ? meta.softClass : 'bg-soft'}`}>
                ...
                <Text className="ml-3 flex-1 text-[14px] font-black text-ink">{option.actionLabel}</Text>
  ...
  return (
    <Page>
      ...
      <View className="bg-white px-5 pb-6 pt-4">
        <Text className="text-[9px] font-black text-berry">MY MARKETPLACE</Text>
        <Text className="mt-1 text-[24px] font-black text-ink">내 분양글</Text>
