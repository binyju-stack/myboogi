            ...
            <AnimatedPressable onPress={() => router.push(`/breeder/${breeder.id}` as never)} className="mb-3 rounded-[24px] border border-line bg-white p-4 shadow-sm">
              <View className="flex-row items-center">
                <View>
                  ...
                  {meta?.isNew ? <Text className="absolute -right-1 -top-1 rounded-full bg-berry px-2 py-1 text-[8px] font-black text-white">NEW</Text> : null}
                </View>
                <View className="ml-3 flex-1">
                  <View className="flex-row items-center">
                    <VerifiedBadge label={breeder.verificationBadgeLabel ?? breeder.badge} />
                  </View>
                  <Text className="mt-2 text-[14px] font-black text-ink">{breeder.name}</Text>
                ...
                >
                  <Text className="text-[9px] font-black text-berry">팔로잉</Text>
                </AnimatedPressable>
              </View>
              <View className="mt-4 rounded-[18px] bg-soft px-4 py-3">
                <Text className="text-[10px] font-bold text-muted">최근 활동</Text>
                <Text className="mt-1 text-[12px] font-black text-ink">{meta?.recentActivity ?? '최근 활동 준비중'}</Text>
              </View>
            </AnimatedPressable>
          </FadeInView>
        ))
