  ...
  return (
    <SafeAreaView className="flex-1 bg-page" edges={['top']}>
        ...
        <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 108 + insets.bottom }}>
          <FadeInView>
            <View className="mx-5 mt-5 rounded-[26px] bg-white p-5 shadow-sm">
              <View className="flex-row items-center">
                ...
                <View className="ml-3 flex-1"><Text className="text-[10px] font-black text-[#E45B5B]">REPORT TARGET</Text><Text className="mt-1 text-[17px] font-black text-ink">{targetType}</Text></View>
              </View>
          ...
          <FadeInView delay={60}>
            <View className="mx-5 mt-4 rounded-[26px] bg-white p-5 shadow-sm">
              <Text className="text-[10px] font-black text-berry">REASON</Text><Text className="mt-1 text-[19px] font-black text-ink">신고 사유를 선택해 주세요</Text>
              <View className="mt-4">
                {reportReasons.map((item) => (
                  <AnimatedPressable key={item} onPress={() => setReason(item)} className="mb-2 flex-row items-center rounded-[18px] bg-soft px-4 py-4">
                    ...
                    <Text className="ml-3 text-[12px] font-black text-ink">{item}</Text>
                  </AnimatedPressable>
                ))}
              </View>
            </View>
          ...
          <FadeInView delay={100}>
            <View className="mx-5 mt-4 rounded-[26px] bg-white p-5 shadow-sm">
              <Text className="text-[10px] font-black text-berry">DETAIL</Text><Text className="mt-1 text-[19px] font-black text-ink">상세 내용을 알려주세요</Text>
      ...
      <View className="absolute bottom-0 left-0 right-0 border-t border-line bg-white px-5 pt-3" style={{ paddingBottom: Math.max(insets.bottom, 12) }}>
        <AnimatedPressable onPress={() => setCompleteVisible(true)} className="items-center rounded-[18px] bg-berry py-4"><Text className="text-[14px] font-black text-white">신고하기</Text></AnimatedPressable>
      </View>
