  ...
  return (
    <Page>
      ...
      <View className="bg-white px-5 pb-6 pt-4">
        <Text className="text-[10px] font-bold text-berry">MYBOOGI NOTICE</Text>
        <Text className="mt-1 text-[24px] font-bold text-ink">마이부기의 소식이에요</Text>
          ...
          <FadeInView key={notice.id} delay={index * 50}>
            <AnimatedPressable onPress={() => router.push(`/notices/${notice.id}`)} className="mb-3 rounded-[24px] bg-white p-5 shadow-sm">
              <View className="flex-row items-center">
                {notice.important ? <Text className="rounded-full bg-blush px-2.5 py-1.5 text-[9px] font-bold text-berry">중요 공지</Text> : <Text className="rounded-full bg-soft px-2.5 py-1.5 text-[9px] font-bold text-muted">공지</Text>}
                <Text className="ml-auto text-[9px] text-subtle">{notice.date}</Text>
              </View>
              <Text className="mt-4 text-[14px] font-bold leading-6 text-ink">{notice.title}</Text>
