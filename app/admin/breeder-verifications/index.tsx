...
export default function BreederVerificationAdminScreen() {
  const waiting = breederVerifications.filter((item) => item.status === '대기').length;
          ...
          <FadeInView key={item.id} delay={index * 55}>
            <View className="mb-3 rounded-[24px] bg-white p-5 shadow-sm">
              <View className="flex-row items-center">
                ...
                <View className="ml-3 flex-1"><Text className="text-[14px] font-black text-ink">{item.breederName}</Text><Text className="mt-1 text-[9px] text-muted">{item.applicantName} · {item.appliedAt}</Text></View>
