  ...
  return (
    <Page>
      ...
      <View className="px-4 pb-5 pt-4">
        ...
        <View className="mt-3 flex-row gap-2"><Pressable className="flex-1 flex-row items-center justify-center rounded-xl border border-line py-3"><Ionicons name="call-outline" size={17} color={colors.ink} /><Text className="ml-1.5 text-xs font-black text-ink">전화문의</Text></Pressable><Pressable className="flex-1 flex-row items-center justify-center rounded-xl bg-[#FEE500] py-3"><Ionicons name="chatbubble" size={17} color={colors.ink} /><Text className="ml-1.5 text-xs font-black text-ink">카카오톡 문의</Text></Pressable></View>
      </View>
      <View className="flex-row border-y border-line">{['판매중', '분양완료', '후기', '대표 개체'].map((label, index) => <View key={label} className={`flex-1 items-center py-4 ${index === 0 ? 'border-b-2 border-berry' : ''}`}><Text className={`text-xs font-bold ${index === 0 ? 'text-berry' : 'text-muted'}`}>{label}</Text></View>)}</View>
