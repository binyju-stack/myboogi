...
export default function TabLayout() {
  ...
  return (
    <Tabs screenOptions={{ headerShown: false, animation: 'fade', transitionSpec: { animation: 'timing', config: { duration: 160 } }, tabBarActiveTintColor: colors.berry, tabBarInactiveTintColor: colors.subtle, tabBarShowLabel: true, tabBarLabelPosition: 'below-icon', tabBarLabelStyle: { width: '100%', textAlign: 'center', fontSize: 11, fontWeight: '500', lineHeight: 14, marginTop: 4, marginBottom: 0, paddingBottom: 0, includeFontPadding: false }, tabBarIconStyle: { width: '100%', height: 28, marginTop: 0, marginBottom: 0, alignItems: 'center', justifyContent: 'center' }, tabBarItemStyle: { flex: 1, height: 64, alignItems: 'center', justifyContent: 'center', paddingTop: 6, paddingBottom: 4, marginHorizontal: 0 }, tabBarStyle: { height: 80 + bottomInset, minHeight: 80 + bottomInset, paddingTop: 8, paddingBottom: bottomInset, paddingHorizontal: 0, borderTopWidth: 1, borderTopColor: '#F1F3F5', backgroundColor: colors.white, overflow: 'visible', ...shadows.bar } }}>
      {tabs.map((tab) => <Tabs.Screen key={tab.name} name={tab.name} options={{ title: tab.title, tabBarLabel: tab.title, tabBarIcon: ({ color, focused }) => <TabIcon focused={focused} color={color} icon={tab.icon} activeIcon={tab.activeIcon} badgeCount={tab.name === 'chat' ? unreadChatCount : 0} /> }} />)}
