...
export default function CommunityScreen() {
  return (
    <Page>
      ...
      <View className="mx-5">{posts.map((post) => <PostCard key={post.id} item={post} />)}</View>
      <Pressable onPress={() => router.push('/disease')} className="absolute bottom-24 right-5 h-14 w-14 items-center justify-center rounded-full bg-berry shadow-lg"><Ionicons name="create" size={23} color="white" /></Pressable>
    </Page>
  );
}
