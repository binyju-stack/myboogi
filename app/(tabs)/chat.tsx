...
function ChatRoomCard({ room, index }: { room: ChatRoom; index: number }) {
  return (
    <FadeInView delay={index * 35}>
      <AnimatedPressable onPress={() => router.push(`/chat/${room.id}`)} className="flex-row border-b border-[#EEF2F6] bg-white px-5 py-4">
        <View>
        ...
        <View className="ml-3 flex-1" style={{ minWidth: 0 }}>
          <View className="flex-row items-center">
            <Text className="flex-1 text-[16px] font-bold text-[#111827]" numberOfLines={1}>{room.participantName}</Text>
            <Text className="ml-2 text-[11px] font-medium text-[#94A3B8]">{room.lastMessageAt}</Text>
          </View>
          <Text className="mt-1.5 text-[13px] font-medium leading-5 text-[#94A3B8]" numberOfLines={1}>{room.lastMessage}</Text>
          <View className="mt-3 flex-row items-center">
            ...
            <View className="ml-2 flex-1" style={{ minWidth: 0 }}>
              <Text className="text-[10px] font-bold text-[#94A3B8]">관련 개체</Text>
              <Text className="mt-0.5 text-[12px] font-bold text-[#FF2E6F]" numberOfLines={1}>{room.listingTitle ?? '일반 상담'}</Text>
            </View>
            {room.unreadCount ? (
    ...
    <Page>
      <View className="border-b border-line bg-white px-5 pb-5 pt-4">
        <View className="flex-row items-center justify-between">
          <View>
            <Text className="text-[10px] font-bold text-berry">MYBOOGI CHAT</Text>
            <Text className="mt-1 text-[26px] font-bold text-[#111827]">채팅</Text>
          </View>
        ...
        <Text className="mt-3 text-[12px] font-medium leading-5 text-[#94A3B8]">분양 문의, 사육 상담, 사진 상담을 한곳에서 확인해요.</Text>
      </View>
