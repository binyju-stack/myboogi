      ...
      <View className="flex-1 justify-end bg-black/35 px-4 pb-6">
        ...
        <View className="rounded-[28px] bg-white p-4 shadow-lg">
          ...
          <AnimatedPressable onPress={onReport} className="flex-row items-center rounded-[18px] px-3 py-4">
            ...
            <View className="ml-3 flex-1"><Text className="text-[13px] font-black text-ink">신고하기</Text><Text className="mt-1 text-[9px] text-muted">운영정책에 맞지 않는 콘텐츠를 알려주세요.</Text></View>
          ...
          {canBlock ? (
            <AnimatedPressable onPress={onBlock} className="mt-1 flex-row items-center rounded-[18px] px-3 py-4">
              ...
              <View className="ml-3 flex-1"><Text className="text-[13px] font-black text-ink">차단하기</Text><Text className="mt-1 text-[9px] text-muted">해당 사용자의 콘텐츠를 더 이상 보지 않아요.</Text></View>
              <Ionicons name="chevron-forward" size={16} color={colors.subtle} />
            </AnimatedPressable>
          ) : null}
          <AnimatedPressable onPress={onClose} className="mt-2 items-center rounded-[18px] bg-soft py-4"><Text className="text-[12px] font-black text-muted">취소</Text></AnimatedPressable>
        </View>
      </View>
    </Modal>
  );
